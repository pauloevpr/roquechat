...
export function useIdb<Definition extends WireStoreDefinition>(
	...
	async function deleteDatabase(): Promise<void> {
		...
		return new Promise((resolve, reject) => {
			...
			request.onerror = (e: any) => reject(`Deleting db '${name}' failed: ${e.target.error}`)
			request.onblocked = (e: any) => reject(`Deleting db '${name}' failed due to blocking: ${e.target.error}`)
		})
	}
